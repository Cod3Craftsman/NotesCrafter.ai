...
function FinalResult({ result }) {
  ...
  if (!result || !result.subTopics || !result.questions || !result.questions.short || !result.questions.long || !result.revisionPoints) return null
  ...
  return (
    <div className='mt-6 p-3 space-y-10 bg-white'>
      ...
      <div className='flex flex-col md:flex-row md:items-center md:justify-between gap-4'>
        ...
        <div className='flex gap-3'>
          <button
            ...
            className={`cursor-pointer px-4 py-2 rounded-lg text-sm font-medium transition ${quickRevision
              ? "bg-green-600 text-white"
              : "bg-green-100 text-green-700 hover:bg-green-200"
              }`}>

            {quickRevision ? "Exit Revision Mode" : "Quick Revision Mode"}
          </button>
          <button className='px-4 py-2 rounded-lg text-sm font-medium bg-indigo-600 text-white hover:bg-indigo-700 cursor-pointer'>
            Download Pdf
          </button>
        </div>
      </div>
      ...
      {result?.diagram?.data && <section>
        ...
        <p className='mt-3 text-xs  to-gray-500 italic'>📸If you need this diagram for future reference or revision,
          feel free to save it by taking a screenshot.
        </p>
      ...
      {result.charts?.length > 0 &&
        <section>
          ...
          <p className='mt-3 text-xs text-gray-500 italic'>📸If you need this Chart for future reference or revision,
            you can save it by taking a screenshot.</p>
        </section>}
      ...
      <section>
        <SectionHeader icon="🙋🏻‍♂️" title="Important Questions" color="rose" />
        ...
        <p className='mt-5 font-semibold'>Long Questions:</p>
        <ul className='list-disc ml-6 text-gray-700'>
        ...
        <p className='mt-5 font-semibold'>Diagram Question:</p>
